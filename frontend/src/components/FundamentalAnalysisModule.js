...
function FundamentalAnalysisModule() {
  ...
  const analyzeFundamentals = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/fundamentals-perplexity/${symbol}`);
  ...
  const addToComparison = async (symbolToAdd) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/fundamentals-perplexity/${symbolToAdd}`);
